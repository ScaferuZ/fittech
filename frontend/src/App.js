...
function App() {
  ...
  const checkAuthState = async () => {
    try {
      ...
      if (currentUser) {
        ...
        try {
          const currentRec =
            await recommendationService.getCurrentRecommendation(
              currentUser.uid
            );
          if (currentRec) {
            console.log(
              "✅ Found existing recommendation on app load:",
              currentRec
            );
  ...
  const handleUserSubmit = async (formData) => {
    ...
    try {
      ...
      const userProfile =
        recommendations.user_metrics || recommendations.user_profile || null;
                ...
                <HStack spacing={0} py={2} minW="max-content">
                  {[
                    ...
                    <Button
                      ...
                      colorScheme={
                        location.pathname === item.path ? "brand" : "gray"
                      }
              ...
              <Route
                ...
                element={
                  <RecommendationPage
                    ...
                    onNewRecommendation={() => navigate("/input")}
