  ...
  async saveRecommendation(
    ...
    try {
      ...
      const recommendationData = {
        ...
        calculatedNutrition: this.calculateNutritionFromTemplate(
          userData,
          userProfile,
          recommendations
        ),
  ...
  calculateNutritionFromTemplate(userData, userProfile, recommendations) {
    try {
      ...
      const templateId =
        ...
        recommendations.predicted_nutrition_template_id;
      ...
      return {
        ...
        targetProtein:
          ...
          Math.round(userWeight * 2),
        targetCarbs:
          nutrition.target_carbs ||
          nutrition.carbs_grams ||
          Math.round(userWeight * 3),
        targetFat:
          nutrition.target_fat ||
          nutrition.fat_grams ||
          Math.round(userWeight * 1),
  ...
  async setOtherRecommendationsInactive(userId, currentRecommendationId) {
    try {
      ...
      const updatePromises = snapshot.docs
        ...
        .map((doc) => setDoc(doc.ref, { isActive: false }, { merge: true }));
    ...
    try {
      ...
      if (userDoc.exists() && userDoc.data().currentRecommendationId) {
        const recommendationRef = doc(
          db,
          "recommendations",
          userDoc.data().currentRecommendationId
        );
    ...
    try {
      ...
      await setDoc(
        recommendationRef,
        {
          ...
          feedbackCount:
            (await this.getFeedbackCount(feedbackData.recommendationId)) + 1,
        },
        { merge: true }
      );
    ...
    try {
      ...
      return {
        ...
        targetCalories: Math.round(
          tdee * (nutrition.caloric_intake_multiplier || 0.8)
        ),
