...
const RecommendationDisplay = ({
  ...
  const calculateDailyMacros = () => {
    ...
    const tdee =
      userProfile.tdee || userProfile.total_daily_energy_expenditure || 2000;
    ...
    if (
      ...
      nutrition.target_carbs &&
      nutrition.target_fat
    ) {
    ...
    if (
      nutrition.caloric_intake_multiplier &&
      nutrition.protein_per_kg &&
      nutrition.carbs_per_kg &&
      nutrition.fat_per_kg
    ) {
          ...
          if (dailyMacros) {
            ...
            if (mealPlanResult.success) {
              const transformedMealPlan =
                mealPlanService.transformMealPlanToFrontend(mealPlanResult);
              setMealPlan(transformedMealPlan);
            } else {
              console.warn(
                "Failed to load organized meal plan, using fallback"
              );
              setMealPlan(null);
            }
            ...
            try {
              console.log(
                "🔄 Fetching meal plan from backend with macros:",
                dailyMacros
              );
              ...
              if (backendPlan && backendPlan.meal_plan && onMealPlanGenerated) {
                console.log(
                  "🔄 Storing meal plan for recommendation history..."
                );
                onMealPlanGenerated(backendPlan.meal_plan);
              }
            } catch (backendError) {
              console.error(
                "❌ Failed to fetch backend meal plan:",
                backendError
              );
    ...
    return (
      <Box textAlign="center" py={10}>
        ...
        <Text color="gray.600" mb={6}>
          Silakan isi formulir profil terlebih dahulu untuk mendapatkan
  ...
  const calculateFoodPortions = (targetMacros) => {
    ...
    const mealDistribution = {
      ...
      makan_siang: { percentage: 0.4, name: "☀️ Makan Siang", foodCount: 4 },
      makan_malam: { percentage: 0.3, name: "🌙 Makan Malam", foodCount: 3 },
    ...
    Object.entries(mealDistribution).forEach(([mealType, config]) => {
      ...
      if (selectedFoods.length < config.foodCount) {
        const additionalFoods = nutritionData
          .filter(
            (food) =>
      ...
      const mealFoods = selectedFoods
        ...
        .map((food, index) => {
          ...
          if (mealType === "camilan") {
            ...
            finalGrams = Math.min(
              100,
              Math.max(30, (baseCalories * 100) / food.calories)
            );
          } else {
            ...
            finalGrams = Math.min(
              350,
              Math.max(50, (baseCalories * 100) / food.calories)
            );
          }
          ...
          return {
            ...
            actualProtein:
              Math.round((food.protein / 100) * finalGrams * 10) / 10,
      ...
      const totalActualCalories = mealFoods.reduce(
        (sum, food) => sum + food.actualCalories,
        0
      );
      ...
      if (Math.abs(calorieRatio - 1) > 0.15) {
        mealFoods.forEach((food) => {
          ...
          food.grams = Math.min(
            mealType === "camilan" ? 150 : 400,
            Math.max(30, adjustedGrams)
          );
          food.actualCalories = Math.round((food.calories / 100) * food.grams);
          food.actualProtein =
            Math.round((food.protein / 100) * food.grams * 10) / 10;
          food.actualCarbs =
            Math.round((food.carbs / 100) * food.grams * 10) / 10;
      ...
      suggestions.push({
        ...
        actualCalories: mealFoods.reduce(
          (sum, food) => sum + food.actualCalories,
          0
        ),
        actualProtein:
          Math.round(
            mealFoods.reduce((sum, food) => sum + food.actualProtein, 0) * 10
          ) / 10,
        actualCarbs:
          Math.round(
            mealFoods.reduce((sum, food) => sum + food.actualCarbs, 0) * 10
          ) / 10,
        actualFat:
          Math.round(
            mealFoods.reduce((sum, food) => sum + food.actualFat, 0) * 10
          ) / 10,
        foods: mealFoods,
      });
    });
  ...
  const transformBackendMealPlan = (backendPlan) => {
    ...
    Object.entries(backendPlan.daily_meal_plan).forEach(
      ([mealType, mealData]) => {
        if (mealData && mealData.foods) {
          ...
          const mealTargets =
            backendPlan.meal_targets && backendPlan.meal_targets[mealType];

          const targetCalories = mealTargets
            ? Math.round(mealTargets.calories)
            : Math.round(mealData.scaled_calories);
          const targetProtein = mealTargets
            ? Math.round(mealTargets.protein)
            : Math.round(mealData.scaled_protein);
          const targetCarbs = mealTargets
            ? Math.round(mealTargets.carbs)
            : Math.round(mealData.scaled_carbs);
          const targetFat = mealTargets
            ? Math.round(mealTargets.fat)
            : Math.round(mealData.scaled_fat);

          // Ensure we have valid numbers (fallback to scaled values if targets are missing)
          const finalTargetCalories = isNaN(targetCalories)
            ? Math.round(mealData.scaled_calories)
            : targetCalories;
          const finalTargetProtein = isNaN(targetProtein)
            ? Math.round(mealData.scaled_protein)
            : targetProtein;
          const finalTargetCarbs = isNaN(targetCarbs)
            ? Math.round(mealData.scaled_carbs)
            : targetCarbs;
          const finalTargetFat = isNaN(targetFat)
            ? Math.round(mealData.scaled_fat)
            : targetFat;
  ...
  const foodSuggestions =
    backendMealPlan && backendMealPlan.daily_meal_plan
      ? transformBackendMealPlan(backendMealPlan)
      : mealPlan && mealPlan.length > 0
      ? mealPlan
      : dailyMacros
      ? calculateFoodPortions(dailyMacros)
      : [];
          ...
          <CardBody>
            ...
            >
              ...
              <Stat>
                ...
                <StatNumber>
                  {userData?.fitness_goal === "Fat Loss"
                    ? "Membakar Lemak"
                    : userData?.fitness_goal === "Muscle Gain"
                    ? "Menambah Massa Otot"
                    : userData?.fitness_goal === "Maintenance"
              ...
              <Stat>
                ...
                <StatNumber>
                  {userData?.activity_level === "Low Activity"
                    ? "Aktivitas Rendah"
                    : userData?.activity_level === "Moderate Activity"
                    ? "Aktivitas Sedang"
                    : userData?.activity_level === "High Activity"
                    ? "Aktivitas Tinggi"
            ...
            <CardBody>
              ...
              >
                <Stat>
                  ...
                  <StatNumber>
                    {recommendations.user_profile.bmi?.toFixed(1)}
                  </StatNumber>
                  <StatHelpText>
                    <Badge
                      colorScheme={
                        recommendations.user_profile.bmi_category === "Normal"
                          ? "green"
                          : "orange"
                      }
                    >
                      ...
                      "Underweight"
                        ...
                        : recommendations.user_profile.bmi_category ===
                          "Overweight"
                        ? "Kelebihan Berat"
                        : recommendations.user_profile.bmi_category === "Obese"
                        ? "Obesitas"
                        : recommendations.user_profile.bmi_category}
                    </Badge>
                  </StatHelpText>
                ...
                <Stat>
                  ...
                  <StatNumber>
                    {Math.round(recommendations.user_profile.bmr)} kkal
                  </StatNumber>
                ...
                <Stat>
                  ...
                  <StatNumber>
                    {Math.round(recommendations.user_profile.tdee)} kkal
                  </StatNumber>
                ...
                <Stat>
                  ...
                  <StatNumber>
                    {recommendations.user_profile.bmi_category === "Underweight"
                      ? "Kurus"
                      : recommendations.user_profile.bmi_category === "Normal"
                      ? "Normal"
                      : recommendations.user_profile.bmi_category ===
        ...
        {workout && (
          <Card>
            <CardHeader>
              ...
              {/* Reference link for more workout info */}
              <Box mt={2}>
                <Text fontSize={{ base: "xs", md: "sm" }} color="blue.600">
                  Untuk informasi lebih lengkap tentang latihan, silakan
                  kunjungi:{" "}
                  <a
                    href="https://exrx.net/Lists/WorkoutMenu"
                    target="_blank"
              ...
              <VStack spacing={{ base: 4, md: 6 }} align="stretch">
                  ...
                  <Box>
                    ...
                    <SimpleGrid columns={2} spacing={3}>
                      <Stat>
                        ...
                        <StatNumber>
                          {workout.workout_type === "Full Body"
                            ? "Seluruh Tubuh"
                            : workout.workout_type === "Upper/Lower Split"
                            ? "Split Atas/Bawah"
                            : workout.workout_type === "Push/Pull/Legs"
                            ? "Dorong/Tarik/Kaki"
                      ...
                      <Stat>
                        ...
                        <StatNumber>
                          {workout.days_per_week || 3} hari
                        </StatNumber>
                      </Stat>
                      <Stat>
                        <StatLabel>Set Harian</StatLabel>
                        <StatNumber>
                          {workout.sets_per_exercise || 3} set
                        </StatNumber>
                      </Stat>
                      <Stat>
                        <StatLabel>Latihan per Sesi</StatLabel>
                        <StatNumber>
                          {workout.exercises_per_session || 5} latihan
                        </StatNumber>
                      </Stat>
                    </SimpleGrid>
                  </Box>
                    ...
                    <Box>
                      ...
                      <SimpleGrid columns={2} spacing={3}>
                        <Stat>
                          ...
                          <StatNumber>
                            {workout.cardio_minutes_per_day} menit
                          </StatNumber>
                        </Stat>
                        <Stat>
                          <StatLabel>Sesi per Hari</StatLabel>
                          <StatNumber>
                            {workout.cardio_sessions_per_day || 1} sesi
                          </StatNumber>
                        </Stat>
                      </SimpleGrid>
                    </Box>
                ...
                {(workout.workout_schedule ||
                  ...
                  <Box>
                    ...
                    {/* Reference link for more workout info, styled as Alert */}
                    <Alert status="info" mt={4} borderRadius="md" bg="blue.50">
                      <AlertIcon />
                      <Box flex="1">
                        <Text
                          fontSize={{ base: "xs", md: "sm" }}
                          color="blue.800"
                        >
                          Untuk informasi lebih lengkap tentang latihan dan
                          contoh jadwal, kunjungi:
                          <a
                            href="https://exrx.net/Lists/WorkoutMenu"
                            target="_blank"
                            rel="noopener noreferrer"
                  ...
                  >
                    <Box p={4} borderWidth={1} borderRadius="md">
                      ...
                      <Text fontSize={{ base: "xs", md: "sm" }}>
                        Melatih semua kelompok otot utama dalam setiap sesi.
                        Cocok untuk pemula dan orang dengan waktu terbatas.
                      </Text>
                    </Box>
                    <Box p={4} borderWidth={1} borderRadius="md">
                      <Text
                      ...
                      <Text fontSize={{ base: "xs", md: "sm" }}>
                        Split latihan atas dan bawah tubuh. Cocok untuk atlet
                        menengah dengan ketersediaan waktu sedang.
                      </Text>
                    </Box>
                    <Box p={4} borderWidth={1} borderRadius="md">
                      <Text
                      ...
                      <Text fontSize={{ base: "xs", md: "sm" }}>
                        Split berdasarkan gerakan dorong, tarik, dan kaki. Cocok
              ...
              <VStack spacing={{ base: 4, md: 6 }} align="stretch">
                <Box>
                  <Heading size={{ base: "xs", md: "sm" }} mb={4}>
                    🎯 Target Harian Berdasarkan Template
                  </Heading>
                  <SimpleGrid
                    columns={{ base: 1, sm: 2, md: 4 }}
                    spacing={{ base: 3, md: 4 }}
                        ...
                        >
                          {userData?.fitness_goal === "Fat Loss"
                            ? `Defisit: ${Math.round(
                                (1 -
                                  dailyMacros.calories /
                                    (recommendations?.user_profile?.tdee ||
                                      2000)) *
                                  100
                              )}%`
                            : userData?.fitness_goal === "Muscle Gain"
                  ...
                  foodSuggestions.length > 0 && (
                    <Box>
                      <Heading size={{ base: "xs", md: "sm" }} mb={4}>
                        🍽️{" "}
                        {mealPlan
                          ? "Rencana Makan Berdasarkan Template"
                          : "Porsi Makanan Indonesia Berdasarkan Template"}
                      </Heading>
                      <Text
                        fontSize={{ base: "xs", md: "sm" }}
                        color="gray.600"
                      ...
                      >
                        {mealPlan
                          ...
                          : "Porsi yang dihitung untuk mencapai target nutrisi harian Anda"}
                      </Text>
                        ...
                        {foodSuggestions.map((meal, index) => (
                          ...
                          >
                            ...
                            <Text
                              ...
                              mb={3}
                            >
                              Target: {meal.targetCalories} kkal |{" "}
                              {meal.targetProtein}g protein | {meal.targetCarbs}
                              g carbs | {meal.targetFat}g fat
                              {meal.actualCalories && (
                                <Text as="span" color="blue.600" ml={2}>
                                  • Aktual: {meal.actualCalories} kkal |{" "}
                                  {meal.actualProtein}g protein |{" "}
                                  {meal.actualCarbs}g carbs | {meal.actualFat}g
                                  fat
                                </Text>
                              )}
                            </Text>
                                ...
                                >
                                  ...
                                  <SimpleGrid columns={4} spacing={2}>
                                    <Text fontSize={{ base: "2xs", md: "xs" }}>
                                      🔥 {food.actualCalories} kkal
                                    </Text>
                                    <Text fontSize={{ base: "2xs", md: "xs" }}>
                                      🥩 {food.actualProtein}g
                                    </Text>
                                    <Text fontSize={{ base: "2xs", md: "xs" }}>
                                      🍞 {food.actualCarbs}g
                                    </Text>
                      ...
                      {/* Total Summary */}
                      {foodSuggestions.length > 0 &&
                        foodSuggestions[0].actualCalories && (
                          <Box mt={4} p={4} bg="blue.50" borderRadius="md">
                            ...
                            >
                              <Stat>
                                <StatLabel fontSize={{ base: "2xs", md: "xs" }}>
                                  Total Kalori
                                </StatLabel>
                                <StatNumber fontSize={{ base: "sm", md: "md" }}>
                                  {foodSuggestions.reduce(
                                    (sum, meal) =>
                                      sum + (meal.actualCalories || 0),
                                    0
                                  )}{" "}
                                  kkal
                                </StatNumber>
                            ...
                            <Box mt={4}>
                              ...
                              >
                                {(() => {
                                  const totalActualCalories =
                                    foodSuggestions.reduce(
                                      (sum, meal) =>
                                        sum + (meal.actualCalories || 0),
                                      0
                                    );
                                  const totalActualProtein =
                                    foodSuggestions.reduce(
                                      (sum, meal) =>
                                        sum + (meal.actualProtein || 0),
                                      0
                                    );
                                  const totalActualCarbs =
                                    foodSuggestions.reduce(
                                      (sum, meal) =>
                                        sum + (meal.actualCarbs || 0),
                                      0
                                    );
                                  const totalActualFat = foodSuggestions.reduce(
                                    (sum, meal) => sum + (meal.actualFat || 0),
                                    0
                                  );
                                  ...
                                  const proteinAccuracy = Math.round(
                                    (totalActualProtein / dailyMacros.protein) *
                                      100
                                  );
                                  const carbAccuracy = Math.round(
                                    (totalActualCarbs / dailyMacros.carbs) * 100
                                  );
                                  const fatAccuracy = Math.round(
                                    (totalActualFat / dailyMacros.fat) * 100
                                  );
                ...
                {!backendMealPlanLoading && backendMealPlan && (
                  <Box>
                    ...
                    <Text
                      ...
                      mb={4}
                    >
                      Rencana makan yang dihasilkan oleh AI berdasarkan template
                      nutrisi dan kebutuhan kalori Anda
                    </Text>
                      ...
                      <Box mb={4} p={4} bg="blue.50" borderRadius="md">
                        ...
                        <SimpleGrid columns={{ base: 2, md: 4 }} spacing={3}>
                          <Stat>
                            ...
                            <StatNumber fontSize={{ base: "sm", md: "md" }}>
                              {Math.round(
                                backendMealPlan.daily_summary.total_calories
                              )}{" "}
                              kkal
                            </StatNumber>
                          </Stat>
                          <Stat>
                            ...
                            <StatNumber fontSize={{ base: "sm", md: "md" }}>
                              {Math.round(
                                backendMealPlan.daily_summary.total_protein
                              )}
                              g
                            </StatNumber>
                          </Stat>
                          <Stat>
                            ...
                            <StatNumber fontSize={{ base: "sm", md: "md" }}>
                              {Math.round(
                                backendMealPlan.daily_summary.total_carbs
                              )}
                              g
                            </StatNumber>
                          </Stat>
                          <Stat>
                            ...
                            <StatNumber fontSize={{ base: "sm", md: "md" }}>
                              {Math.round(
                                backendMealPlan.daily_summary.total_fat
                              )}
                              g
                            </StatNumber>
                          </Stat>
                    ...
                    {backendMealPlan.meals && (
                      <VStack spacing={4} align="stretch">
                        {Object.entries(backendMealPlan.meals).map(
                          ([mealType, mealData]) => (
                            <Box
                              key={mealType}
                              p={4}
                            ...
                            >
                              ...
                              <SimpleGrid columns={4} spacing={2} mb={3}>
                                <Text fontSize={{ base: "2xs", md: "xs" }}>
                                  🔥 {Math.round(mealData.calories)} kkal
                                </Text>
                                <Text fontSize={{ base: "2xs", md: "xs" }}>
                    ...
                    {/* Shopping List */}
                    {backendMealPlan.shopping_list &&
                      backendMealPlan.shopping_list.length > 0 && (
                        <Box mt={4} p={4} bg="orange.50" borderRadius="md">
                          ...
                          <SimpleGrid
                            ...
                            spacing={2}
                          >
                            {backendMealPlan.shopping_list.map(
                              (item, index) => (
                                <Flex
                                  key={index}
                                  justify="space-between"
                ...
                {backendMealPlanLoading && (
                  <Box p={6} textAlign="center" bg="blue.50" borderRadius="md">
                    ...
                    <Text fontSize={{ base: "sm", md: "md" }}>
                      Sedang menghasilkan rencana makan yang dipersonalisasi
                      dengan AI...
                    </Text>
                  </Box>
                )}
                ...
                {mealPlanLoading && (
                  <Box p={6} textAlign="center" bg="green.50" borderRadius="md">
                    <Heading size={{ base: "sm", md: "md" }} mb={2}>
                      🔄 Memuat Rencana Makan...
                    </Heading>
                    <Text fontSize={{ base: "sm", md: "md" }}>
                      Sedang menyusun kombinasi makanan yang optimal untuk
                      Anda...
                    </Text>
                  </Box>
                )}
